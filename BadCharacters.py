...
def _file_too_large(view):
    if view.size() > DEFAULT_MAX_FILE_SIZE:
        sublime.status_message(
            "File too large - BadCharacters plugin disabled.")
        return True
...
class BadCharactersListener(sublime_plugin.EventListener):
    ...
    def highlight_bad_characters(self, view):
        ...
        if zerowidth_regions:
            view.add_regions('BadCharactersListener', zerowidth_regions,
                             'invalid.illegal', '', sublime.DRAW_EMPTY)
        if invalid_regions:
            view.add_regions('BadCharactersListener', invalid_regions,
                             'invalid.illegal', '', sublime.DRAW_EMPTY)
